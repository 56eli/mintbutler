...
- Standing rule (owner 2026-09-17): "yes always provide stub after recommendations" — every behavior recommended in an orchestrator prompt must be backed by stub-based harness tests in the same PR. Applied via boilerplate in every module prompt from 007 onward: stubs simulate external tools/commands (sudo/apt/dconf/xdg-mime/timeshift/xrandr/lp/inxi/pactl...), fake HOMEs, failing-tool variants for error paths; real system access (elevated installs, hardware) remains owner-acceptance only.
